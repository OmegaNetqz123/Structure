...
```
Error: locator.waitFor: Error: strict mode violation: locator('text=Processing...') resolved to 2 elements:
    1) <span>Processing...</span> aka getByText('Processing...').first()
    2) <span>Processing...</span> aka getByText('Processing...').nth(1)

Call log:
  - waiting for locator('text=Processing...') to be hidden
    ...
    - generic [ref=e170]:
      - generic [ref=e172]:
        - generic [ref=e173]:
          ...
          - textbox [ref=e175]: Module1
        - generic [ref=e176]:
          - generic [ref=e177]: Module ID
          - textbox [ref=e178]: ID001
        - generic [ref=e179]:
...
```ts
  ...
  63 |     const processing = page.locator('text=Processing...'); // adjust selector to match exact DOM
  64 |     // it might not even appear yet, so don't fail if it's absent
  65 |     await processing.waitFor({ state: "visible", timeout: 5000 }).catch(() => {});
> 66 |     await processing.waitFor({ state: "hidden", timeout: 60000 });
     |                      ^ Error: locator.waitFor: Error: strict mode violation: locator('text=Processing...') resolved to 2 elements:
  67 | }
  68 | 
```