...
```ts
  ...
  216 |             const shouldBeChecked = desiredValue.toString().trim().toLowerCase() === 'yes';
  217 |             const checkbox = clms.locator('span');
  218 |             const isCurrentlyChecked = await checkbox.isChecked();
  219 |             if (shouldBeChecked && !isCurrentlyChecked) {
  220 |                 await checkbox.check();
  221 |             }
  222 |             else if (!shouldBeChecked && isCurrentlyChecked) {
